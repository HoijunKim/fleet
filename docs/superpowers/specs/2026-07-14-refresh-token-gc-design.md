...
- **Prune EXPIRED only.** `DELETE FROM refresh_tokens WHERE expires_at < now()`. Revoked-but-not-expired rows are RETAINED - they are the slice-4b reuse-detection tripwires for an active family; deleting them would weaken reuse detection. Deleting an expired row is safe because an expired token can never mint access (`rotateOnce` rejects it on the expiry check, and a revoked+expired token 401s without minting), while every NON-expired revoked tripwire is retained - so detection of any ACTIVE compromise is preserved. NOTE: rotation extends expiry (each new token gets a fresh TTL, `auth/handlers.go`), so an ancestor can be expired while its family's tip is still live; the only thing lost by pruning is the alarm for a stale replay of an already-expired token, which can grant nothing anyway. This matches the OAuth 2.0 Security BCP (drop reuse-detection records at token expiry).
